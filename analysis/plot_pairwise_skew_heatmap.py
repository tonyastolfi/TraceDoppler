...
def calculate_pairwise_skew_stats(tracedata, type="raw"):
    ...
    for trace in tracedata:
        ...
        while bfs_queue:
            ...
            parent_correction = int(
                get_attribute_from_tags(parent, "clock_skew_correction")
            )
            ...
            for child in children:
                ...
                child_correction = int(
                    get_attribute_from_tags(child, "clock_skew_correction")
                )
                child_skew = int(get_attribute_from_tags(child, "global_skew_ns"))

                pairwise_skew[parentService][childService].append(
                    parent_skew - child_skew
                )
                skew_corrections[parentService][childService].append(
                    parent_correction - child_correction
                )
                error = (
                    parent_skew - child_skew + (parent_correction - child_correction)
                )
                pairwise_error[parentService][childService].append(error)

                # reflect
                pairwise_skew[childService][parentService].append(
                    child_skew - parent_skew
                )
                skew_corrections[childService][parentService].append(
                    child_correction - parent_correction
                )
                error = (
                    child_skew - parent_skew + (child_correction - parent_correction)
                )
                pairwise_error[childService][parentService].append(error)

    for service in services:
        for service2 in services:
            pairwise_skew[service][service2] = np.array(
                pairwise_skew[service][service2]
            )
            skew_corrections[service][service2] = np.array(
                skew_corrections[service][service2]
            )
            pairwise_error[service][service2] = np.array(
                pairwise_error[service][service2]
            )
...
vmax = (
    np.max(
        [
            np.max(np.mean(list(pairwise_skew[service][service2]) + [1.0]))
            for service in services
            for service2 in services
        ]
    )
...
heatmap2, ticks = calculate_heatmap(pairwise_error)
sns.heatmap(
    heatmap2,
    annot=True,
    fmt=".2f",
